...
<script>
import { listRoleStatus,listRole, getRole, delRole, addRole, updateRole, dataScope, changeRoleStatus } from "@/api/system/role";
...
export default {
  ...
  methods: {
    ...
    handleUpdate(row) {
      ...
      this.form.status = row.role_status.toString();
    },
    /** 分配数据权限操作 */
    handleDataScope(row) {
      this.reset();
